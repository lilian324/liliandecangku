...
def start_tsne(x_train, y_train):
    ...
    x_train1 = x_train.reshape(len(x_train), 81)
...
def end_tsne(x_test, y_test, model):
    model.load_state_dict(torch.load('../model/2DCNN.pth'))
    model.eval()
    with torch.no_grad():
...
def data_pre():
    ...
    x_train = pad_and_reshape(x_train)
    x_valid = pad_and_reshape(x_valid)
    x_test = pad_and_reshape(x_test)
...
# 填充函数
def pad_and_reshape(data, target_shape=(1, 9, 9)):
    padded_data = []
    for sample in data:
        sample = np.pad(sample, (0, target_shape[1] * target_shape[2] - sample.shape[0]), 'constant')
        reshaped_sample = sample.reshape(target_shape)
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_accuracy:
            ...
            torch.save(model.state_dict(), '../model/2DCNN.pth')
